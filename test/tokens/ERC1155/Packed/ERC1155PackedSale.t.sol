...
import {ERC1155PackedSale} from "src/tokens/ERC1155/Packed/ERC1155PackedSale.sol";
import {ERC1155PackedSaleFactory} from "src/tokens/ERC1155/Packed/ERC1155PackedSaleFactory.sol";
...
contract ERC1155PackedSaleTest is Test, ERC1155SaleErrors, ERC1155SupplyErrors {
    ...
    function setUpFromFactory() public {
        ERC1155PackedSaleFactory factory = new ERC1155PackedSaleFactory();
        token = ERC1155PackedSale(factory.deployERC1155Sale(address(this), "test", "ipfs://", ""));
    }
    ...
    function testDebugMe(bool useFactory, address mintTo, uint256 tokenId, uint256 amount)
        public
        assumeSafe(mintTo, tokenId, amount)
        withFactory(useFactory)
        ...
        withTokenSaleActive(tokenId)
    {
        ...
        uint256 count = token.balanceOf(mintTo, tokenId);
        vm.expectEmit(true, true, true, true);
        emit TransferBatch(address(this), address(0), address(69), tokenIds, amounts);
        token.mint{value: amount * perTokenCost}(address(69), tokenIds, amounts, "");
        assertEq(count + amount, token.balanceOf(address(69), tokenId));
    }
